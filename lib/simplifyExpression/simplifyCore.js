...
const {wrapObject} = require('../util/myUtil')
 ...
 * @return {*[]} Returns the simplified form of `expr`
 */
  ...
  while (!visited[str]) {
    ...
    for (let i = 0; i < rules.length; i++) {
      if (typeof rules[i] === 'function') {
        ...
        flatten(res)
        tempRes = applyRule(res, rules[i], scope, options)
        newRes = tempRes && tempRes.length > 0 ? tempRes[0] : null
      }

      if (newRes) {
        steps.push( Object.assign({}, newRes, { nodeAfterAlts:tempRes.map((res) => res.nodeAfter) }))
...
function findRuleMatchForEachSplittedNode(rule, node, scope, options) {
  if(node.args && rule && node.args.length >= 2){
    const splits = getSplits(node, node.context)

    let resArray = []
    for (let i = 0; i < splits.length; i++)
      resArray.push(_ruleMatch(rule, splits[i], false, scope, options))

    resArray = resArray.map((res) => res && res.length > 0 ? res : null).filter((res) => res !== null)
...
function applyRule(node, rule, scope, options) {
  ...
  let mat = findRuleMatchForEachSplittedNode(rule.l, res, false, scope, options) // If the rule is associative operator, we can try matching it while allowing additional terms.
  // This allows us to match rules like 'n+n' to the expression '(1+x)+x' or even 'x+1+x' if the operator is commutative.

  if (!mat && rule.expanded) {
    repl    = rule.expanded.r
    mat = findRuleMatchForEachSplittedNode(rule.expanded.l, res, false, scope, options)
  }
  if (mat) {
    mat.forEach((ma) => {
      ma.forEach((m)=>{
        ...
        // only add if the nodeAfter.toString() hasn't been found yet
        const hasBeenFound =  alt.find((alt) => alt.nodeAfter.toString() === res.toString())
        if(!hasBeenFound && res){
          alt.push({
            ruleApplied: rule,
            nodeBefore: node,
...
function getSplits(node, context) {
  ...
  // lets print it.const tmpSplitMemory = new Set()
  // if(Array.isArray(res) && res.length > 0) {
  //   const allToString = res.map((node) => node.toString()).join(',\n')
  //   tmpSplitMemory.add(allToString)
  //   console.log('done')
  //   const printableMemory =  Array.from(tmpSplitMemory).join('---\n\n\n')
  //   console.log(printableMemory)
  // }
