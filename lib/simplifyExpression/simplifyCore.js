...
import { createMakeNodeFunction, flatten, hasOwnProperty, isAssociative, isCommutative, isConstantNode, resolve, unflattenl, unflattenr } from '~/util/utilTemp.js'
...
function simplify(expr, rules, scope, options) {
  const exprStr = myNodeToString(expr)
  ...
  const key = `${exprStr}|${rulesKeyPart}|${scopePart}|${optionsPart}`
  if (cache.has(key)) {
  ...
  if (options.getMistakes)
    rules = buildAndApplyMistakeRules(rules, scope, options) // attaches mistake rules to the rules

  let res = resolve(expr)
  const visited = {}

  let str = myNodeToString(res, {
    parenthesis: 'all',
  })
  let newRes = null
  options = options || {}
  while (!visited[str]) {
    ...
    for (let i = 0; i < rules.length; i++) {
      ...
      if (false) {
        const mistakeReses = []
        for (const mistakeRule of rules[i].mistakeRules) {
          const ruleToUse = {
            ...rules[i],
            // r: mistakeRule?.r,
...
function buildAndApplyMistakeRules(rules, scope, options) {
  if (!rules || rules.length === 0)
    return rules

  rules?.forEach?.((rule) => {
    if (rule.mistakes) {
      const buildMistakeRules = []
      rule.mistakes.forEach((mistake) => {
        const rule = mistake.getFromRule ? mistake.getFromRule() : mistake
        const builtRule = _buildRules([rule], scope, options)[0]
        buildMistakeRules.push(builtRule)
      })
      rule.mistakeRules = buildMistakeRules
    }
  })
  return rules
}
