 ...
 */
export function findMultiplyBeforeAddMistakes(expressionString: string): string[] {
  ...
  const alternativeInterpretations: Set<string> = new Set()

  // Function to safely replace part of the string while considering context
  function generateAlternative(expression: string, match: string, startIndex: number, endIndex: number, calculatedPart: string) {
...
export function mistakeSearches(start: string) {
  const addBeforeMultMistakes: ProcessedStep[] = []
...
function processMatches(regex: RegExp, str: string, handler: { (match: RegExpExecArray): void }) {
