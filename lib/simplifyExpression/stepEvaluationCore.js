...
function makeStepsWithInfo(res, lastStep, userStep, startingStepAnswer, availableChangeTypes = []) {
  availableChangeTypes = availableChangeTypes.flat()
  ...
  if (history.length === 0) {
    ...
    newSteps = history.map((evaledStep, index) => {
      ...
      if (isLastStep && isFoundStepAMistake) {
        ...
        return { isValid: false, reachesOriginalAnswer: false, from: lastFrom, to: res.mToStep, attemptedToGetTo: to, attemptedChangeType: evaledStep.changeType, mistakenChangeType, availableChangeTypes }
      }
      ...
      return { isValid: true, reachesOriginalAnswer: hasSameEventualAnswer, from: lastFrom, to, attemptedToGetTo: to, attemptedChangeType: evaledStep.changeType, mistakenChangeType: null, availableChangeTypes }
    })
  }
