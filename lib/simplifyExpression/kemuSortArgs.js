    ...
    if (node.args) {
      ...
      switch (node.op) {
        case '*': {
          ...
          // ---Grayson added I think this is supposed to be here
...
function allAlternateAdditionForms(node){
  // This function is used to generate all possible addition forms of a given node
  // ex. a+b+c, b+a+c, c+a+b,
  // only adds what can logically be added
  // ex. a + b * c has none
  //  a + b + (c * d) has b + a + (c * d) & (c * d) + a + b & (c * d) + b + a
