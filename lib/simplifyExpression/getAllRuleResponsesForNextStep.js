import simplifyCommon from './_common.js'
...
import { setUpNewNodeTree, travelTreeExpanded } from '../util/treeUtil.js'
import { getFirstOperationChoicePermutations } from '../util/nodePermutations.js'
...
function getAllResponsesForRule(fct, root, options) {
  ...
  travelTreeExpanded(root, (_node, context) => {
    ...
    for (const newNode of rulesReses) {
      ...
      // if (newNode.mLRes) {
      //   for (const mlNewNode of newNode.mLRes)
      //     getNodeFromTopNodeHack({ context, newNode: mlNewNode, ogTopNode, ogRootNode }, resultStr => errorResultLog.add({ to: resultStr, changeType: mlNewNode.changeType }))
      // }
    }
  })
  if (!changeTypeFound || !resultLog.size === 0)
    return null
  return { changeType: changeTypeFound, to: Array.from(resultLog), mTo: Array.from(errorResultLog), from: myNodeToString(root), rootNode: root }
}
