...
const {getSplits} = require('../../util/myUtil')
...
function temp(node, fn){
  const from = node.toString()
  const nodeSplit = getSplits(node)//  rootNodeAlts: oneStep.nodeAfterAlts,
  const res = nodeSplit.map((node) => {
    const rv = fn(simplifyCommon.kemuFlatten(node))
    if(rv && rv.rootNode){
      rv.resToFromJsonLog = JSON.stringify({from, to: rv.rootNode.toString(), changeType: rv.changeType})
      return rv
    }
  }).filter((x) => x)

  const rv = res.length > 0 ? res[0] : null
  if(!rv)
    return rv
  rv.rootNodeAlts = res.map((x) => x.rootNode)
  return rv
}

function ogFunction(node){
  let rv = null
...
module.exports = (node)=> temp(node, ogFunction)
