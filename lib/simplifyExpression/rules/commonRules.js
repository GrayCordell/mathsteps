...
const poolOfRules = [
  ...
  // Find common denominator before fractions add.
  {
    ...
    replaceFct(node, vars) {
      ...
      const denominator1 = vars.c2
      const denominator2 = vars.c4
      // Find lowest common multiplier (LCM).
      const lcm = Node.Creator.constant(math.lcm(denominator1.value, denominator2.value))
...
export const commonRules = createFunctionForEveryRule(poolOfRules)
export const commonRulesPooledTogether = node => applyRules(node, poolOfRules)
export default {
  commonRules,
