import  { isCommutative, isAssociative, createMakeNodeFunction, flatten, unflattenr, unflattenl } from '~/util/utilTemp.js';
...
function getSplits(node, context) {
    ...
    if (isCommutative(node, context)) {
        for (let i = 0; i < node.args.length; i++) {
            ...
            let nestedSplits = getSplits(node.args[i], context);
            // Include the original argument as well
            nestedSplits.push(node.args[i]);
            for (let nestedSplit of nestedSplits) {
        ...
        let nestedSplits = getSplits(node.args[0], context);
...
function myToString(unknownThing) {
...
function cleanerMyToString(equation) {
    const eqAsString = myToString(equation);
    // Define the regex pattern to match number * variable
    const pattern = /(\d+)\s*\*\s*([a-zA-Z]+)/g;
