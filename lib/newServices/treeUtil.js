...
export function printTree(node, prefix = '', isLeft = true, isStart = true) {
  const newNode = node
  if (!newNode)
    return
  if (isStart) {
...
export function travelTree(node, fn, context = {}) {
  context = context || {}
...
export function setUpNewNodeTree(context, newNode, newTopNode = null) {
  if (!newTopNode)
    newTopNode = newNode
...
export function travelTreeExpanded(node, fn, context = {}) {
  if (!context.visited) {
...
export function removeImplicitMultiplicationFromNode(node) {
